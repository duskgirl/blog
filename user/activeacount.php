...
function activeUser(){
  ...
  if($mt != $token) {
    header('Location:/blog/user/vertify.php?register=1');
  }
  ...
  if(time()-strtotime($row['modified_time']) > 60){
    ...
    if($result == 1) {
      // 表示超时
      header('Location:/blog/user/sendmail-success.php?time_out=1');
    ...
    <div class="blog_activeacount">
      <div class="cover">
        <div class="container">
          ...
          <div class="alert alert-success success">
            <?php echo $message_success; ?><strong><a href="/blog/user/login.php"> 点此登录</a></strong> 
          </div>
          <?php endif?>
